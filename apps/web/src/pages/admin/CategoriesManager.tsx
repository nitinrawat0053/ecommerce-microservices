...
export default function CategoriesManager({ title = 'Categories', subtitle = 'Manage product categories' }: { title?: string; subtitle?: string }) {
  ...
  const isImageIcon = (src?: string) =>
    !!src && (/^(https?:\/\/|\/|data:image)/i.test(src) || /\.(png|jpe?g|svg|webp|gif)(\?.*)?$/i.test(src));

  const renderIcon = (cat: any) => {
    const name = cat.name || '';
    if (categoryIcons[name.toLowerCase()]) {
      return <span className="text-xl">{categoryIcons[name.toLowerCase()]}</span>;
    }
    ...
    return <span className="text-xl">{cat.icon || '📁'}</span>;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {deletingCategory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeletingCategory(null)}>
          <Card className="w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}>
            <CardContent className="p-6">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={confirmDelete} className="bg-red-600 hover:bg-red-700 text-white gap-2">
